  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      <mesh position={lineCenter} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial color={hovered ? "#ffffff" : "#FF1E41"} transparent opacity={hovered ? 0.9 : 0.5} />
      </mesh>
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        <coneGeometry args={[0.015, 0.04, 8]} />
        <meshBasicMaterial color={hovered ? "#FF1E41" : "#bb0029"} />
      </mesh>
      <group ref={imageGroupRef} position={topPosition}>
        <Html transform center sprite distanceFactor={10} style={{ pointerEvents: isVisible ? "auto" : "none", opacity: isVisible ? 1 : 0, transition: "opacity 0.15s ease-out" }}>
          <div
            className={cn("cursor-pointer overflow-hidden rounded-full shadow-lg transition-transform duration-200 border-2 border-white", hovered && "scale-125 shadow-xl")}
            style={{ width: "10px", height: "10px", background: "#FF1E41" }}
  ...
  return (
    <group>
      ...
      {markers.map((marker, i) => (
        <Marker key={`${i}-${marker.lat}-${marker.lng}`} marker={marker} radius={config.radius} defaultSize={config.markerSize} onClick={onMarkerClick} onHover={onMarkerHover} />
      ))}
    </group>
  );
}
...
const defaultConfig: Required<Globe3DConfig> = {
  ...
  showAtmosphere: true, atmosphereColor: "#FF1E41", atmosphereIntensity: 0.4, atmosphereBlur: 3,
  bumpScale: 1, autoRotateSpeed: 0.4, enableZoom: false, enablePan: false, minDistance: 5, maxDistance: 15,
  initialRotation: { x: 0, y: 0 }, markerSize: 0.06, showWireframe: false, wireframeColor: "#FF1E41",
  ...
  return (
    <div className={cn("relative h-[500px] w-full", className)}>
      <Canvas gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }} dpr={[1, 2]} camera={{ fov: 45, near: 0.1, far: 1000, position: [0, 0, mergedConfig.radius * 3.5] }} style={{ background: mergedConfig.backgroundColor || "transparent" }}>
        <Suspense fallback={<Html center><span className="text-sm text-zinc-400">Loading globe…</span></Html>}>
